import { useDirectory, getTopSkill } from '@/hooks/useDirectory';
import { COUNTRIES, SKILLS_CATALOG } from '@/types';
import Header from '@/components/Header';
import { Card, CardContent, CardHeader, CardTitle } from '@/components/ui/card';
import { Badge } from '@/components/ui/badge';
import { Users, Globe, Award, GraduationCap, Briefcase, TrendingUp, Star, MapPin, Loader2 } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const countryData = COUNTRIES
    .map(c => ({ name: c, count: profiles.filter(p => p.country === c).length }))
    .filter(d => d.count > 0)
    .sort((a, b) => b.count - a.count);

  const skillData = SKILLS_CATALOG
    .map(s => ({ name: s, count: profiles.filter(p => p.skills.some(sk => sk.skill_name === s)).length }))
    .filter(d => d.count > 0)
    .sort((a, b) => b.count - a.count);

  const pregradoCount = profiles.filter(p => p.education.some(e => e.type === 'Pregrado')).length;
  const posgradoCount = profiles.filter(p => p.education.some(e => e.type === 'Posgrado')).length;
  const eduData = [{ name: 'Pregrado', value: pregradoCount }, { name: 'Posgrado', value: posgradoCount }];

  const totalCerts = profiles.reduce((sum, p) => sum + p.certifications.length, 0);
  const totalProjects = profiles.reduce((sum, p) => sum + p.projects.length, 0);

  const COLORS = ['hsl(215, 60%, 22%)', 'hsl(178, 55%, 40%)', 'hsl(40, 80%, 55%)', 'hsl(145, 60%, 42%)', 'hsl(205, 75%, 55%)'];

  const stats = [
    { icon: Users, label: 'Total Perfiles', value: totalProfiles, color: 'text-accent' },
    { icon: Globe, label: 'Países', value: countryData.length, color: 'text-info' },
    { icon: Star, label: 'Fortalezas', value: skillData.length, color: 'text-gold' },
                ...
                {profiles.slice(0, 5).map(p => {
                  ...
                  return (
                    <div key={p.id} className="flex items-center gap-3">
                      ...
                      {topSkill && <Badge variant="secondary" className="text-xs">{topSkill.skill_name}</Badge>}
                    </div>
                  );
                })}
              </div>
