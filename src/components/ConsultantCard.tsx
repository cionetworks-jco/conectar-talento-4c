...
import { getTopSkill, getLatestEducation, getLatestCertification } from '@/data/mockData';
...
export default function ConsultantCard({ profile }: Props) {
  ...
  const topExpertise = profile.skills.length > 0
    ? Math.max(...profile.skills.map(s => s.expertise))
    : 0;
